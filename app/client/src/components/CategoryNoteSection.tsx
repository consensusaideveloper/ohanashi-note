...
const INITIAL_ACCUMULATIVE_VISIBLE = 3;
...
/** Inline SVG icon for "view conversation" buttons. */
      ...
      {reversed.map((version, index) => (
        ...
        >
          ...
          {onViewConversation !== undefined && (
            <button
              type="button"
              className="inline-flex items-center gap-1 min-h-11 px-3 py-1.5 rounded-full bg-bg-surface-hover text-text-secondary text-base font-medium hover:bg-border-light active:bg-border transition-colors"
...
/** Card-based display for accumulative entries (all entries as equal peers). */
function AccumulativeEntryCards({
  entry,
  onViewConversation,
}: {
  ...
  const visibleEntries =
    showAll || totalCount <= INITIAL_ACCUMULATIVE_VISIBLE
      ? entries
      : entries.slice(0, INITIAL_ACCUMULATIVE_VISIBLE);
  const hiddenCount = totalCount - visibleEntries.length;

  return (
    <div className="border-l-2 border-accent-secondary/30 pl-3">
      <div className="flex items-center gap-1.5">
        <span className="inline-flex items-center justify-center min-w-6 h-6 rounded-full bg-accent-secondary/20 text-accent-secondary text-base font-semibold px-1.5">
          {totalCount}
        </span>
      ...
      <div className="mt-2 space-y-2 pl-1">
        {visibleEntries.map((item: AccumulativeEntry, index) => (
          <div
            key={`${item.conversationId}-${item.recordedAt}-${index}`}
            className="bg-bg-surface-hover rounded-xl px-3 py-2.5"
          >
            ...
            <div className="mt-1.5 flex items-center gap-2">
              ...
              {onViewConversation !== undefined && (
                <button
                  type="button"
                  className="inline-flex items-center gap-1 min-h-11 px-3 py-1 rounded-full text-accent-primary text-base font-medium hover:bg-accent-primary/10 active:bg-accent-primary/20 transition-colors"
        ...
        {hiddenCount > 0 && (
          <button
            type="button"
            className="w-full min-h-11 rounded-full border border-border-light text-text-secondary text-base font-medium px-4 py-2 hover:bg-bg-surface-hover active:bg-border-light transition-colors"
...
/** Single-value entry display (current answer + optional version history). */
function SingleEntryDisplay({
  entry,
  expandedHistories,
  onViewConversation,
  onHistoryToggle,
  ...
  onViewConversation?: (id: string) => void;
  onHistoryToggle: (e: React.MouseEvent<HTMLButtonElement>) => void;
}): ReactNode {
  const handleViewClick = useCallback(
    (e: React.MouseEvent<HTMLButtonElement>): void => {
      const convId = e.currentTarget.dataset["conversationId"];
  ...
  return (
    <div className="border-l-2 border-accent-primary/30 pl-3">
      ...
      {entry.hasHistory && expandedHistories.has(entry.questionId) && (
        <div className="pl-5">
          <VersionTimeline
              ...
              {data.noteEntries.map((entry) =>
                entry.questionType === "accumulative" ? (
                  ...
                  <SingleEntryDisplay
                    ...
                    onViewConversation={onViewConversation}
                    onHistoryToggle={handleHistoryToggleClick}
                  />
                ),
              )}
            </div>
