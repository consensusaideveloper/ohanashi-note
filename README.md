...
補足:
- 初回設定が完了するまでは、通常のノート会話は開始できません
- 初回設定中の会話は設定案内専用であり、エンディングノートとして保存・分析されません
- ユーザーが設定中に本題へ誘導しても、AIは設定完了を優先して案内します
...
| データのエクスポート | ノートPDF・会話PDFをZIPで一括ダウンロード（録音同梱は任意、期間指定可） |
...
- **代表者の設定**: 家族メンバーの中から「代表者」を指定（最大3名）。代表者はノートのライフサイクル操作（逝去報告、同意収集の開始など）を行える
