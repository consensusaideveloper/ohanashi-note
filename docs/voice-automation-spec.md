...
初回設定未完了ユーザー（`users.onboarding_completed_at IS NULL`）に対して、フォーム入力ではなく音声会話でセットアップを行う。
...
| `update_assistant_name` | 話し相手の呼び名設定 | Tier 1 |
| `change_font_size` | 文字サイズ選択 | Tier 1 |
| `update_speaking_preferences` | 話し方の好み設定 | Tier 1 |
| `complete_onboarding` | 初回設定完了の確定 | Lifecycle |
...
4. 話し相手の呼び名を確認し、必要なら `update_assistant_name` で設定
5. 文字サイズの好み（標準/大きめ/特大）を確認 → `change_font_size` で設定
6. 話し方の好み（速さ・待ち時間・確認頻度）を確認 → `update_speaking_preferences` で設定
7. `complete_onboarding` で設定完了を確定し、AIが設定内容を読み上げて確認
8. `end_conversation` で会話終了 → 完了画面を経てメインアプリへ遷移
9. 設定中にユーザーが「本題（ノート関連の話題）」を話した場合は内容をノート保存せず、必要ならローカルに一時メモして初回通常会話への話題引き継ぎ候補としてのみ扱う

### ガードレール

- オンボーディング会話の最優先目的は初回設定完了であり、AIは自分からノート本題の質問を始めてはいけない
- 最初の質問は必ず名前確認に固定する
- ユーザーが思い出・医療・お金・家族・葬儀などの本題へ誘導しても、1〜2文で受け止めて設定案内へ戻す
- オンボーディングでは通常会話用の「まだ聞いていない質問から聞く」進行を無効化する
- サーバー側でも、初回設定未完了ユーザーの通常会話セッションは `ONBOARDING_REQUIRED` で拒否する

### セッション制限免除

- オンボーディング会話はデイリーセッション上限にカウントされない
- Realtime 接続時に `onboarding=true` フラグを付与
- サーバー側で `users.onboarding_completed_at IS NULL` のときだけ正規のオンボーディングとして扱い、セッション上限を免除
- オンボーディング会話は設定案内専用とし、会話内容をエンディングノートとして保存・要約・分析しない（設定値のみ保存）
- 初回設定完了は汎用プロフィール更新ではなく、オンボーディングセッション `sessionKey` を要求する専用 API でのみ記録する
...
- `server/src/routes/realtime.ts` — オンボーディング接続のセッション制限免除と通常会話ブロック
- `server/src/routes/profile.ts` — 汎用プロフィール更新（オンボーディング完了フラグは更新不可）
