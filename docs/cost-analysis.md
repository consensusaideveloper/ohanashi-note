...
| **推奨料金** | 年額¥9,800・全機能込み・家族無料（粗利率96%） |
...
| 家族利用 | 11（本人のみ） | **¥419** | 家族は無料（限界コスト≒0） |
...
| **コスト構造** | 1セッション原価¥33と極めて安い。月3回フル利用でも粗利率87.9%を維持でき、上位プランで原価を回収する必要がない |
...
> 原価が¥33/セッションと非常に低いため、月3回フル利用でも**87.9%の粗利率**を維持できる。プレミアムプランで回収する必要がそもそもない。
...
| 月次セッション上限 | なし | 未実装（プラン導入時に新設予定） | - | 料金プラン導入時に実装 |
...
```
損益分岐点 = ¥4,109,000 / ¥9,435 = 436ユーザー
```

> 人件費込みで**約436ユーザーが損益分岐点**。

#### スケール別収益見通し

| ユーザー数 | 年間売上 | 年間変動費 | 年間固定費 | 営業利益 |
|-----------|---------|----------|----------|---------|
| 100 | ¥980,000 | ¥36,500 | ¥4,109,000 | **-¥3,165,500** |
| 436 | ¥4,272,800 | ¥159,140 | ¥4,109,000 | **¥4,660** |
| 1,000 | ¥9,800,000 | ¥365,000 | ¥4,149,000 | **¥5,286,000** |
| 3,000 | ¥29,400,000 | ¥1,095,000 | ¥4,649,000 | **¥23,656,000** |
...
7. プラン別ストレージ上限の実装（プラン確定後）
8. 無料トライアルの実装（14日間、2セッション）
9. `last_active_at`追加・非アクティブアカウント検知
