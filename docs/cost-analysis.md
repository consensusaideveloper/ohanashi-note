...
| 項目 | 内容 |
|------|------|
| **1セッション（20分）の原価** | **約¥37.4（$0.249、音声アップロード成功時）** |
| **コストの89%** | 音声会話（gpt-realtime-mini）が占める |
| **標準ユーザーの年間原価** | 約¥413（11セッション） |
| **インフラ固定費** | 月¥750〜¥3,000（規模依存） |
| **最大のリスク** | データの無期限蓄積（自動クリーンアップなし） |
| **直接的な競合** | 存在しない（AI音声会話型は当サービスが世界初の可能性） |
| **推奨料金** | 年額¥9,800・全機能込み・家族無料（粗利率95%台） |

### コスト構造の概観

```
1セッション（音声アップロード成功時）= ¥37.4 の内訳:

gpt-realtime-mini（音声会話）  ¥32.40  ███████████████████████████████  89.2%
gpt-4o-mini-transcribe（再文字起こし）  ¥3.60  ███  9.6%
gpt-5-mini（要約）              ¥0.35  ▏  0.9%
gpt-5-nano（TODO生成）           ¥0.13  ▏  0.4%
ストレージ（R2+DB）             微小    ▏  <0.1%
```

> **注**: 現行実装では、録音アップロードに成功した会話は `enhanced-summarize` を自動実行する（`useConversation.ts`）。そのため実運用の標準原価は約¥37.4。音声アップロード失敗時のみフォールバックで約¥33.8。
...
| 会話要約 | gpt-5-mini | `services/summarizer.ts` | $0.25/1M tokens | $2.00/1M tokens |
| 高精度再文字起こし | gpt-4o-mini-transcribe | `services/transcriber.ts` | 音声入力: $3.00/1M audio tokens（~$0.003/分） | - |
...
| 項目 | トークン数 | 単価 | コスト |
|------|-----------|------|--------|
| 入力（システムプロンプト+トランスクリプト） | ~3,000 tokens | $0.25/1M | $0.00075 |
| 出力（構造化JSON） | ~800 tokens | $2.00/1M | $0.00160 |
| **合計** | | | **$0.00235（¥0.35）** |

> トランスクリプトの最大入力は30,000文字（`MAX_TRANSCRIPT_CHARS`）。gpt-5-miniへ上げても1セッションあたりの増分は約¥0.28で、精度向上の効果に対して十分小さい。

### 2.4 高精度再文字起こし（gpt-4o-mini-transcribe）

`enhanced-summarize`エンドポイント経由で実行。現行フローでは録音アップロード成功時に自動実行される（失敗時のみ通常要約へフォールバック）。
...
| gpt-4o-mini-transcribe | 再文字起こし（標準: 音声アップロード成功時） | $0.024 | ¥3.6 | 9.6% |
| gpt-5-mini | 会話要約 | $0.0024 | ¥0.35 | 0.9% |
| gpt-5-nano | TODO生成（オンデマンド） | $0.0009 | ¥0.13 | 0.4% |
| **合計（音声アップロード成功時）** | | **$0.249** | **¥37.4** | **100%** |
| **合計（音声アップロード失敗時フォールバック）** | | **$0.225** | **¥33.8** | - |
...
**エンディングノートサービスでは「大切な記録を残すこと」が本質的な価値である。** ユーザーが認識しないうちにデータが消えるのはサービスの存在意義を損なう。

- **ユーザーに見えるデータは自動削除しない** — 音声、会話テキスト、要約、メタデータは全てユーザーが明示的に削除するまで保持
- **コスト管理は料金プランの容量制限で行う** — 自動削除ではなくプランごとのストレージ上限で制御
- **内部中間データのみ自動クリーンアップ** — ユーザーに見えない処理用データは安全に削除可能

### 4.2 データ分類

| 分類 | フィールド | サイズ/会話 | 自動削除 | 説明 |
|------|-----------|-----------|---------|------|
| **ユーザーデータ** | transcript | ~15KB | **しない** | 会話全文。オーナー・家族が閲覧 |
| | summary | ~1KB | **しない** | 会話の要約 |
| | noteEntries | ~3KB | **しない** | エンディングノートの核心データ |
| | keyPoints | ~1KB | **しない** | 重要ポイント |
| | oneLinerSummary | ~100B | **しない** | 一覧表示用の一行要約 |
| | topicAdherence | ~10B | **しない** | 分析メタデータ |
| | offTopicSummary | ~200B | **しない** | 分析メタデータ |
| | 音声ファイル (R2) | ~500KB | **しない** | 故人の声は家族にとって替えがたい価値 |
| **内部中間データ** | improvedTranscript | ~15KB | **する** | enhanced-summarize処理の中間成果物。どのAPIにも返されない |
| | transcriptionModel | ~30B | **する** | improvedTranscriptのメタデータ |

### 4.3 実装済み: 内部中間データの自動クリーンアップ（`data-retention.ts`）

6時間間隔の定期ジョブで`improvedTranscript`のみをクリーンアップ。
...
- **削減効果**: 会話あたり ~15KB（DB肥大化を防止）

### 4.4 コスト管理戦略: 料金プランによる容量制限

ストレージコストはデータの自動削除ではなく、料金プランのストレージ上限で管理する。ユーザーが上限に近づいた場合は通知し、不要な会話を自分で選んで削除できるようにする。

**既存の削除手段**:
- 個別会話の削除: `DELETE /api/conversations/:id`（R2音声も連動削除）
- アカウント全削除: `DELETE /api/account`（全データカスケード削除）
- ライフサイクル削除: 家族の全員同意による故人データの削除

**将来実装が必要な項目**（プラン確定後）:
- プラン別ストレージ上限の定義と施行
- ユーザーごとのストレージ使用量トラッキング
- 上限接近時のユーザー通知
- 上限超過時の新規会話制限

### 4.5 ストレージコストの長期予測

ユーザーデータを自動削除しないため、ストレージは線形に増加する。コストは料金プランの収益でカバーする前提。

| 時期 | ユーザー数 | R2ストレージ | R2月額 | DBストレージ | DB月額 |
|------|-----------|------------|--------|------------|--------|
| 1年後 | 1,000 | 36GB | $0.54 | 3.5GB | ~$5 |
| 3年後 | 10,000 | ~1.5TB | $22.50 | ~70GB | ~$15 |
| 5年後 | 30,000 | ~8TB | $120 | ~350GB | ~$30 |

> **注**: 10,000ユーザー時のストレージ総コストは ~$37.50/月。年間プラン収益（¥9,800 × 10,000 = ¥9,800万）に対して十分にカバー可能。

### 4.6 将来課題

#### 非アクティブアカウントの処理

長期間利用のないアカウントについては、将来的に以下の段階的処理を検討:

```
6ヶ月未ログイン  → 通知「アカウントが休止状態です」
9ヶ月未ログイン  → 2回目通知「30日後に休止処理します」
10ヶ月未ログイン → 凍結（データ保持、新規会話不可）
...
| gpt-5-mini | 要約 | **低** | 精度重視の抽出タスクに適したバランスモデル。1セッションあたりの増分コストは約¥0.28 |
| gpt-4o-mini-transcribe | 文字起こし | **中** | GPT-4oシリーズの段階的廃止傾向だが、transcribe専用モデルは代替が少ない |
| gpt-5-nano | TODO生成 | **低** | 単価が非常に低く、出力品質も用途に十分 |

### 5.2 GPT-4oシリーズ廃止の経緯

| 日付 | イベント |
|------|---------|
| 2025-09-10 | `gpt-4o-realtime-preview` / `gpt-4o-mini-realtime-preview` の廃止告知（OpenAI Deprecations） |
| 2026-02-27 | `gpt-4o-realtime-preview` / `gpt-4o-mini-realtime-preview` の提供終了（OpenAI Deprecations） |
| 2026-03-03 | `gpt-4o-realtime-preview-2024-12-17` 提供終了（OpenAI Deprecations） |
...
**モデル名の環境変数管理で廃止リスクを即時回避できる構成に統一**

- 対応済み: `OPENAI_*MODEL*` を `config.ts` に集約（`realtime.ts` / `summarizer.ts` / `transcriber.ts` / `todo-generator.ts`）
- 効果: モデル廃止時にコード変更なしで切替可能（環境変数更新のみ）
...
| 会話要約 | gpt-5-mini | **維持** | 精度重視（抽出品質向上） |
| 再文字起こし | gpt-4o-mini-transcribe | **維持・監視** | 代替なし |
| TODO生成 | gpt-5-nano | **維持** | 低コスト・十分品質 |
...
| 項目 | 数値 | 備考 |
|------|------|------|
| 1セッション原価（音声アップロード失敗時フォールバック） | ¥33.8（$0.225） | gpt-realtime-mini + gpt-5-mini |
| 1セッション原価（音声アップロード成功時） | ¥37.4（$0.249） | 再文字起こし含む |
...
```
初月:      5セッション × ¥37.4 = ¥187
2ヶ月目:   3セッション × ¥37.4 = ¥112
3-5ヶ月目: 1セッション × ¥37.4 × 3ヶ月 = ¥112
6-12ヶ月目: 保存のみ
合計: ¥411 + ストレージ ≒ ¥413（年間11セッション）
```

#### ヘビーユーザー

```
1-6ヶ月目:  10セッション × ¥37.4 × 6ヶ月 = ¥2,244
7-12ヶ月目: 2セッション × ¥37.4 × 6ヶ月 = ¥449
合計: ¥2,693 + ストレージ ≒ ¥2,697（年間72セッション）
```
...
```
本人（標準パターン）: ¥413
家族3人（閲覧のみ）: ストレージのみ ~¥54（※家族は無料）
合計: ¥467
```

> **家族メンバーの限界コストは事実上ゼロ**。家族はAIセッション（コストの89%）を使わず、DB読み取り・Firebase Auth（無料枠内）のみ。
...
|---------|-------------|---------|------|
| お試し（離脱） | 3 | **¥114** | 無料トライアルで吸収可能 |
| 標準 | 11 | **¥413** | 基準シナリオ |
| ヘビー | 72 | **¥2,697** | セッション制限が重要 |
| 家族利用 | 11（本人のみ） | **¥467** | 家族は無料（限界コスト≒0） |
...
|-----------|--------|------|---------|----------|
| **楽クラライフノート** | NTTファイナンス | 基本無料（2026-03-05時点、公式FAQ） | エンディングノート、資産管理、家族共有、銀行口座連携 | 50-70代 |
...
1. **年額プラン一本**（行政書士助言）— 高齢者に月額の複雑さを強いない
2. **全機能込み・シンプル構成** — プラン選択の認知負荷をゼロに
3. **家族メンバーは無料** — 限界コスト≒0、課金はサービスの核心価値を損なう
4. **月額換算1,000円以下を意識**（高齢者の価格感度）
5. **セッション数制限**でコスト予測可能に

#### プレミアムプランを設けない理由

| 観点 | 分析 |
|------|------|
| **認知負荷** | 高齢者（60-80歳）に「どのプランが自分に合うか」の判断を求めるのは大きなハードル。選択肢が1つなら迷わず申し込める |
| **コスト構造** | 1セッション原価は約¥37.4（音声アップロード成功時）。月3回フル利用でも粗利率86%台を維持でき、上位プランで原価を回収する必要がない |
| **機能分割の不自然さ** | 家族管理・TODO管理はエンディングノートの本質的機能。これらを上位プランに閉じ込めるとサービスの価値提案が弱まる |
| **初期フェーズの戦略** | ユーザー基盤が小さい段階では、プラン最適化よりも利用者数の最大化が優先。シンプルな価格で導入障壁を下げるべき |
| **将来の柔軟性** | ユーザー基盤が拡大した段階で、データに基づいてプレミアムプランを追加することは容易。逆に複雑な構成をシンプルにするのは難しい |

#### 家族メンバーを無料にする理由

| 観点 | 分析 |
|------|------|
| **限界コスト** | 家族はAIセッション（コストの89%）を一切使わない。DB読み取り+Firebase Auth（無料枠内）で限界コスト≒¥0 |
| **サービスの核心価値** | 「家族に想いを届ける」がエンディングノートの存在意義。家族に課金すると、最も大切な瞬間（故人のノートを開封する時）へのアクセスに障壁を作ることになる |
| **ライフサイクル上の問題** | オーナー死亡後、家族がノートにアクセスするには料金を払い続ける必要がある — これは倫理的に問題 |
| **ビジネスへの貢献** | 家族が無料でアクセスできることで、口コミによる新規ユーザー獲得チャネルとなる |
...
| **想定原価** | ¥37.4 × 2回 × (10分/20分) ≒ **¥37** |

> 2セッション×10分で「話すだけでノートができる」体験を提供。原価は約¥37で依然として低リスク。
...
| **ストレージ** | 無制限 |
| **家族メンバー** | **無料・人数制限なし** |
| **含まれる機能** | 全機能（音声会話、ノート閲覧・編集、会話履歴・検索、キャラクター選択、印刷/PDF、家族管理・ライフサイクル、TODO自動生成、共有リンク） |
...
> 原価が約¥37.4/セッションでも、月3回フル利用で**86.3%の粗利率**を維持できる。プレミアムプランで回収する必要は依然として低い。
...
**価格正当化のポイント**:
- 楽クラライフノートの約2.7倍だが、AI音声会話という唯一無二の機能で差別化
- 全機能込みのシンプル構成 — 高齢者に「どのプランがいいか」の判断を求めない
- 終活コンサルティング1回分（¥5,000-30,000）と比較すれば年額¥9,800は圧倒的にリーズナブル
- NHK受信料（月額¥1,100 = 年額¥13,200）より安い
- ¥10,000未満の心理的価格ライン
...
| 観点 | 分析 |
|------|------|
| **コスト** | 1セッション原価約¥37.4（$0.249）。仮に15分に短縮しても約¥28で、削減額約¥9は料金プランの粗利率（86-95%）に対して影響が小さい |
...
| **1日の最大コスト** | ¥187 | ¥112 | 40%のコスト削減 |
| **月最大コスト（制限なし時）** | ¥5,610 | ¥3,366 | 料金プラン導入前のコスト上限を抑制 |
| **ノート完成ペース** | 5-8日で全110問完了可能 | 8-12日で全110問完了可能 | どちらも十分な速度。高齢者には余裕のあるペースが望ましい |
| **バースト保護** | 1日でコストが急増しやすい | 日次上限で急増を抑制 | 月次制限（3回/月）実装後も、日次上限は短期バースト抑制として有効 |
...
```
     ...
     │ 層2: 日次制限（3回/日）          │ ← バースト防止
     │ 層3: 月次制限（環境変数で有効化） │ ← 収益化後のコスト制御
     └──────────────────────────────────┘
```

現在は層1/層2が常時有効。層3（月次制限）も実装済みで、`MAX_MONTHLY_SESSIONS` を `0` より大きい値（例: 3）に設定した場合のみ有効化される。

#### プランの制限値（将来）

| 制限項目 | 現行（実装済み） | おはなしプラン |
|---------|-------------|-------------|
| セッション時間 | 20分 | 20分 |
| 1日あたり | **3回/日** | 3回/日 |
| 月間セッション | なし（デフォルト無効） | 3回/月（`MAX_MONTHLY_SESSIONS=3`） |
...
| 月次セッション上限 | なし | 実装済み（環境変数で有効化） | 2026-03-05 | `MAX_MONTHLY_SESSIONS` により段階導入可能に |
...
```
インフラ固定費: ¥9,000/年
年間収入/ユーザー: ¥9,800（単一プラン）
平均変動費/ユーザー/年: ¥413
貢献利益/ユーザー/年: ¥9,387

損益分岐点 = ¥9,000 / ¥9,387 = 1ユーザー
```
...
```
損益分岐点 = ¥4,109,000 / ¥9,387 = 438ユーザー
```

> 人件費込みで**約438ユーザーが損益分岐点**。

#### スケール別収益見通し

| ユーザー数 | 年間売上 | 年間変動費 | 年間固定費 | 営業利益 |
|-----------|---------|----------|----------|---------|
| 100 | ¥980,000 | ¥41,300 | ¥4,109,000 | **-¥3,170,300** |
| 438 | ¥4,292,400 | ¥180,894 | ¥4,109,000 | **¥2,506** |
| 1,000 | ¥9,800,000 | ¥413,000 | ¥4,149,000 | **¥5,238,000** |
| 3,000 | ¥29,400,000 | ¥1,239,000 | ¥4,649,000 | **¥23,512,000** |
...
LTV:CAC = 3:1 目標:
```
許容CAC = ¥32,928 / 3 = ¥10,976
```

| チャネル | 推定CPA | 判定 |
|---------|--------|------|
| Google広告 | ¥13,000-33,000 | **赤字** |
| SEO/コンテンツ | ¥2,000-5,000 | **推奨** |
| 行政書士・FP提携 | ¥3,000-5,000 | **推奨** |
| 終活セミナー | ¥1,000-3,000 | **推奨** |
| 家族からの口コミ | ¥0 | **最重要** — 家族無料がこのチャネルを強化 |
...
| 1年目→2年目 | 30%離脱 | 定期見直しリマインド、ライフイベント連動、休止プラン（将来検討） |
...
| ~~**P2**~~ | ~~月次セッションクォータ実装~~ | ~~中~~ | ~~コスト予測性向上~~ | **実装済み（環境変数で有効化）** `session-limits.ts`, `session-quota.ts` |
...
| Prompt Caching（gpt-5-mini要約） | 自動適用 | ~$0.0002/セッション | 効果限定的 |
...
7. Stripe決済の実装
8. ~~月次セッションクォータの実装~~ — **実装済み（`MAX_MONTHLY_SESSIONS` で段階有効化）**

### 中期（3-6ヶ月）

9. プラン別ストレージ上限の実装（プラン確定後）
10. 無料トライアルの実装（14日間、2セッション）
11. `last_active_at`追加・非アクティブアカウント検知
12. 利用状況ダッシュボード（残りセッション数表示）
13. コンビニ払い対応

### 長期（6-12ヶ月）

12. 休止プラン導入（データ保存のみ、料金は将来検討）
...
- [楽クラライフノート FAQ](https://faq.ntt-finance.co.jp/category/show/61?site_domain=rakulife)
...
| `app/server/src/services/summarizer.ts` | 会話要約（gpt-5-mini） |
